...
class TestPS4BC(unittest.TestCase):
    ...
    def test_message_shift_char(self):
        ...
        for char, shift, exp in shifts:
            ...
            self.assertEqual(
                exp,
                result,
                f"Shifting the character '{char}' by {shift} resulted in '{result}' but '{exp}' was expected",
            )
    ...
    def test_plaintext_message_generate_pad(self):
        ...
        for p, exp in plaintexts:
            ...
            self.assertEqual(
                len(exp),
                len(result),
                f"The length of the generated pad is {len(result)} but {len(exp)} was expected",
            )
            self.assertEqual(
    ...
    def test_plaintext_message_pad(self):
        ...
        for p, init_pad, set_pad in pds:
            ...
            self.assertEqual(
                init_pad,
                result,
                f"The PlaintextMessage was initialized with the pad {init_pad} but get_pad() returned {result}",
            )
            ...
            self.assertEqual(
                set_pad,
                result,
                f"The PlaintextMessage pad was changed to {set_pad} but get_pad() returned {result}",
            )
        ...
        self.assertEqual(
            exp,
            result,
            f"The randomly generated PlaintextMessage pad was {result} but {exp} was expected",
        )
    ...
    def test_plaintext_message_ciphertext(self):
        ...
        for p, init_enc, new_pad, new_enc in pds:
            ...
            self.assertEqual(
                init_enc,
                result,
                f"The encrypted message returned was {result} but {init_enc} was expected",
            )
            ...
            self.assertEqual(
                new_enc,
                result,
                f"The encrypted message returned was {result} but {new_enc} was expected",
            )
        random.seed(0)
        new_plaintext = student.PlaintextMessage("Testing generating pad!")
        ...
        self.assertEqual(
            exp,
            result,
            f"The PlaintextMessage encrypted message from a randomly generated pad was {result} but {exp} was expected",
        )
        pad = self.pads[0]
        ...
        self.assertEqual(
            repr_ptmsg_mutated,
            repr(ptmsg_mutated),
            f"Mutating the pad used to initialize PlaintextMessage mutated the PlaintextMessage object. Did you save pad as a copy?",
        )
    ...
    def test_encrypted_message_decrypt(self):
        pds = (
            ...
            (
                "05<Nvv`\\.8_0qvh?b?",
                [59, 44, 50, 90, 86, 13, 13, 60, 64, 46, 70, 74, 45, 86, 5, 93, 15, 57],
                "ThiS iS MixED cASe",
            ),
            (
                "8fxvr6/C'9 _^[G4uXCN",
        ...
        for enc, pad, exp in pds:
            ...
            self.assertEqual(
                exp,
                result,
                f"The decrypted message returned was {result} but {exp} was expected",
            )
    ...
    def test_try_pads(self):
        ...
        for enc, pads, exp_msg, exp_pad in pds:
            ...
            self.assertEqual(
                exp_pad,
                result_pad,
                f"decrypt_message_try_pads returned {result_pad} as the best pad but {exp_pad} was expected",
            )
            self.assertEqual(
                exp_msg,
                result_msg,
                f"decrypt_message_try_pads returned the correct pad and {result_msg} as the decrypted story when {exp_msg} was expected",
            )
        ...
        self.assertEqual(
            [1, 2, 3],
            result_pad,
            "decrypt_message_try_pads should return the last pad if none of them decrypt",
        )
        self.assertEqual(
            "a_a",
            result_msg,
            "decrypt_message_try_pads should return the last pad's decryption if none of them decrypt",
        )
