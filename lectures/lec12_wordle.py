...
def get_word_list(words_str):
    ...
    return words.split("\n")  # VAO: should be words_str?


def is_a_real_word(
    s, word_list
):  # VAO: checks if user input is in list of valid words? or secret word?
    """s is a string
        word_list is a list of words
    Returns True if s is in word_list and False otherwise"""
    return s in word_list


def is_correct_len(guess, length):  # VAO: checks if user input is the correct len
    """guess is a str
        length is an int
...
def play_game():
    ...
    secret = new_word(words)  # VAO: have to actually call it
    wordle_len = 6
    n_guesses = 6
    win = False
    ...
    while guess != secret:
        ...
        # VAO: entire if/else was commented out, why?
        result = make_wordle(guess, secret)
        print(result)
