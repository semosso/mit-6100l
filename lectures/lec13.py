...
# reading
######################################
# finger exercise, p 169: Implement a function that meets the specification below.
# Use a 'try-except' block. Hint: before starting to code, you might want to
# type something like 1 + "a" into the shell to see what kind of exception is raised.
def sum_digits(s):
    ...
    for c in s:
        try:
            ...
            ValueError
        ):  # I had this as type error (the sum), instead of ValueError (the conversion)
            print(f"unable to conver {c} to int")
    return sum
...
# finger exercise, p 175: Implement a function that satisfies the specification
def find_an_even(L):
    ...
    for elem in L:
        if elem % 2 == 0:
            return elem
    raise ValueError("L does not contain an even number")  # no try-except, though
