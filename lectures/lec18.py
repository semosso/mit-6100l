...
class Circle(object):
    def __init__(self, center, radius):
        if type(center) is Coordinate and type(radius) is int:
            self.center = center
            self.radius = radius
        else:
            raise ValueError(
                "invalid types, center has to be Coordinate,radius has to be int"
            )
...
class Fraction(object):
    ...
    def __str__(self):
        ...
        if self.denom == 1:
            return self.num
        else:  # not needed, if you de-indent return below, same thing
            return str(self.num) + "/" + str(self.denom)
...
class Fraction(object):
    ...
    def __pow__(self, other):
        return float(self.num / self.denom) ** float(
            other.num / other.denom
        )  # bullshitty, got it from answer
...
# finger exercise: Write the class according to the specifications below
class Circle:
    ...
    def __str__(self):
        ...
        # your code here
        return f"the radius for circle {self} is {self.r}"  # did it meant only self.r?
