...
test: dict = {"z": 10, "b": 15, "c": 22, "d": "teste"}
test2: dict = {"a": 1, "g": 100, "m": 14, "d": "teste2"}
# print(get_min(test))

# VAO: testing dict aliasing
test.update(test2)
print(test, test2)
print(id(test["a"]), id(test2["a"]), type(test["d"]))

# finger exercise, p 120:
# 1, Remedy the problem described in the previous paragraph. Hint: a simple way
# to do this is to create a new book by appending something to the original book.
# 2, Using `encoder` and `encrypt` as models, implement the functions `decoder` and
# `decript`. Use them to decrypt the message.
...
decoding_book = lambda book, cipher_txt: {
    k: book[int(k)] for k in cipher_txt.split("*")
}
print(decoding_book(don_quixote, message))
# keys = decoding_book(don_quixote, message)
# message_split = message.split("*")


decoder = lambda keys, cipher_txt: "".join([keys[c] for c in (cipher_txt.split("*"))])
# print(decoder(decoding_book, message))

decrypt = lambda book, cipher_txt: decoder(
    keys=decoding_book(book=don_quixote, cipher_txt=message), cipher_txt=message
)

print(decrypt(don_quixote, message))
