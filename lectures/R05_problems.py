...
def rotate_list(input_list, direction, shift):
    # VAO: my code and comments; rotation amount can be index modifier; direction can be +/- in index modifier
    # these comments above are 100% right, but I kept trying to mutate the list in a way, which just can't be done
    # without messing up the reference/base/iterable I need
    # solution suggests slicing based on index
    shift = (
        shift % len(input_list)
    )  # i.e., discarding the excess rotation that moves it back to the original position
    if direction == "left":
...
direction = "right"
print(rotate_list(input_list, direction, 14))
print(
